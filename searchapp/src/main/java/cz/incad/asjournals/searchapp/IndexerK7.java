...
//  https://kramerius-test.lib.cas.cz/search/api/client/v7.0/items/uuid:d118a57e-8d42-4744-9b2b-e76484f4df16/info
  ...
  public void setView(String pid, String ctx, String title, String issueNumber, String volumeNumber, String year) {
    try {
      SolrClient client = getClient("views");
      setView(client, pid, ctx, title, issueNumber, volumeNumber, year);
      client.commit();
      client.close();
  ...
  public void setView(SolrClient client, String pid, String ctx, String title, String issueNumber, String volumeNumber, String year) {
    try {
      ...
      Map<String, Object> ctxModifier = new HashMap<>(1);
      ctxModifier.put("set", ctx);
      idoc.addField("ctx", ctxModifier);
  ...
  public void fillView(SolrInputDocument idoc, String ctx, String title, String issueNumber, String volumeNumber, String year) {
  ...
  public JSONObject fixViews() {
    JSONObject ret = new JSONObject();
    try {
      Options opts = Options.getInstance();
      String solrhost = opts.getString("solr.host", "http://localhost:8983/solr/")
              + "views/export?q=*:*&sort=pid+asc&fl=pid,ctx,title,issueNumber,volumeNumber,year,views";
      ...
      //for (int i = 0; i < 5; i++) {
        ...
        //if (!doc.has("ctx")) {
          String pid = doc.getString("pid");
          JSONObject p = Searcher.getByPid(pid, true);
          JSONObject j = p.getJSONObject("parent");
          if (j.has("doc") && j.getJSONObject("doc").has("root_pid")) {
            JSONObject m = Searcher.getMagazineByPid(j.getJSONObject("doc").getString("root_pid"));
            if (m.has("ctx")) {
              try {
              JSONObject details = details(j);
              ret.put(pid, details);
              SolrInputDocument idoc = new SolrInputDocument();
              idoc.setField("pid", pid);
              fillView(idoc, m.getString("ctx"),
                      p.getJSONObject("doc").optString("title"),
                      details.optString("issueNumber"),
                      details.optString("volumeNumber"),
                      details.optString("year"));
              client.add(idoc, 10);
              } catch (Exception e) {
                LOGGER.log(Level.SEVERE, "Error fixing " + pid, e);
              }
            }
          } 
  ...
  private JSONObject details(JSONObject journal) {
    JSONObject ret = new JSONObject();
    String volumeNumber = null;
    String issueNumber = null;
    
    if (!journal.getJSONObject("doc").has("mods")) {
      return ret;
    }
    JSONObject mods = journal.getJSONObject("doc").getJSONObject("mods");
    String model = journal.getJSONObject("doc").optString("model");
    if ("periodicalvolume".equals(model)) {
      if (mods.has("mods:originInfo")) {
        if (mods.has("mods:titleInfo")) {
          volumeNumber = mods.getJSONObject("mods:titleInfo").get("mods:partNumber").toString();
        }
      }
    } else if ("periodicalitem".equals(model)) {
      if (mods.has("mods:originInfo")) {
        if (mods.has("mods:titleInfo")) {
          issueNumber = mods.getJSONObject("mods:titleInfo").opt("mods:partNumber").toString();
        }
      } else if (mods.has("mods:titleInfo")) {
        issueNumber = mods.getJSONObject("mods:titleInfo").opt("mods:partNumber").toString();
      }
      JSONObject v = details(journal.getJSONObject("parent"));
      volumeNumber = v.optString("volumeNumber");
    }
    ret.put("volumeNumber", volumeNumber);
    ret.put("issueNumber", issueNumber);
    ret.put("year", journal.getJSONObject("doc").opt("year") + "");
    return ret;

  }

  public String exportViews(String ctx) {
    try {
      Options opts = Options.getInstance();
      String solrhost = opts.getString("solr.host", "http://localhost:8983/solr/")
              + "views/export?q=ctx:\"" + ctx + "\"&sort=pid+asc&fl=pid,title,issueNumber,volumeNumber,year,views";
      //        + "views/export?q=*:*&fq=%7B!join%20fromIndex=journal%20to=pid%20from=pid%7Droot_pid:\""+root_pid+"\"&sort=pid+asc&fl=pid,title,issueNumber,volumeNumber,year,views";
