...
export default class Utils {
  ...
  public static findByModel(res: any, model: string): any {
    if (res['doc'].model === model) {
      ...
      return this.findByModel(res['parent'], model);
    } else {
      return null;
    }
  }

  public static journalFromResp(j: any): Journal {
  ...
  public static getDoi(mods: any): string | null {
    if (mods.hasOwnProperty('mods:identifier')) {
      const ids = mods['mods:identifier'];
      if (ids.hasOwnProperty('length')) {
        for (const i in ids) {
          if (ids[i]['type'] === 'doi') {
            return ids[i]['content'];
          }
        }
      } else {
        if (ids['type'] === 'doi') {
  ...
  public static getAutors(mods: any, uppercase: boolean = false): string[] {
    const authors: string[] = [];

    // name/type="personal"	namepart/type="family"
    // name/type="personal"	namePart/type"given"
    if (mods.hasOwnProperty('mods:name')) {
      const name = mods['mods:name'];
      if (name.hasOwnProperty('length')) {
        for (const i in name) {
          const namePart = name[i]['mods:namePart'];
          if (name[i]['type'] === 'personal' && namePart) {
            //Chceme nejdriv prijmeni a potom jmeno
            if (namePart[0]['type'] === 'family') {
              let family: string = namePart[0]['content'];
              if (uppercase) {
                family = family.toUpperCase();
              }
              authors.push(family + ' ' + namePart[1]['content']);
            } else {
              let family: string = namePart[1]['content'];
              if (uppercase) {
                family = family.toUpperCase();
              }
              authors.push(family + ' ' + namePart[0]['content']);
            }
          }
        }
      } else {
  ...
  public static getTitleInfo(titleInfo: any, modsLang: string) {
    let ret = '';
    if (titleInfo.hasOwnProperty('length')) {
      ret = titleInfo[0]['mods:title'];
      for (const i in titleInfo) {
        if (titleInfo[i]['lang'] === modsLang) {
          ret = titleInfo[i]['mods:title'] + '.  ';
          if (titleInfo[i]['mods:subTitle']) {
            ret += titleInfo[i]['mods:subTitle'] + '.  ';
          }
          if (titleInfo[i]['mods:partNumber']) {
            ret += titleInfo[i]['mods:partNumber'] + '.  ';
          }
          if (titleInfo[i]['mods:partName']) {
            ret += titleInfo[i]['mods:partName'] + '.  ';
          }

        }
      }
    } else {
      ret += titleInfo['mods:title'];
      if (titleInfo['mods:subTitle']) {
        ret += titleInfo['mods:subTitle'] + '.  ';
      }
      if (titleInfo['mods:partNumber']) {
        ret += titleInfo['mods:partNumber'] + '.  ';
      }
      if (titleInfo['mods:partName']) {
        ret += titleInfo['mods:partName'] + '.  ';
      }
    }
    return ret;
  }
   ...
   * If not returns null
   */
  public static getJsonValue(json: any, path: string) {
  ...
  private static diacriticsMapInit = false;

  // "what?" version ... http://jsperf.com/diacritics/12
  public static removeDiacritics(str: string) {

    if (!this.diacriticsMapInit) {
